...
const CoursePage = () => {
  ...
  if (course.lessons && url === "") {
    let firstLesson = course?.lessons?.find((e) => e?.order === 1);
      ...
      {course?.lessons ? (
        <div className={s.container}>
          <div>
            ...
            <h3 className={s.description}>Description: <span className={s.thin}>{course.description}</span> </h3>
            ...
            <ul className={s.list}>
              {course.lessons?.map((e) => {
                return (
                  <li key={e.id} className={s.list_item}>
                    Lesson {e.order}: <span className={s.thin}>{e.title}{" "}</span> 
                    {e.status === "locked" && (
